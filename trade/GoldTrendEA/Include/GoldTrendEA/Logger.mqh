...
//+------------------------------------------------------------------+
#ifndef __GTEA_LOGGER_MQH__
#define __GTEA_LOGGER_MQH__
...
private:
   ...
   string            m_dir;               // 日志目录 Files/GoldTrendEA/
   ...
   //--- 写一行日志到当日文件（Files/GoldTrendEA/yyyymmdd.log）并 Print
   void              WriteLine(const ENUM_LOG_LEVEL level, const string msg)
     {
      ...
      // 按日期滚动的日志文件：追加写入 Files/GoldTrendEA/yyyymmdd.log
      int h = FileOpen(m_dir + today, FILE_READ|FILE_WRITE|FILE_TXT|FILE_ANSI);
      if(h == INVALID_HANDLE)
        {
      ...
      if(!m_sizeWarned && fsize > GTEA_LOG_MAX_BYTES)
        {
         m_sizeWarned = true;
         Warn(StringFormat("当日日志文件已超 %d MB, 请检查是否存在告警刷屏",
                           GTEA_LOG_MAX_BYTES / 1024 / 1024));
        }
     }
   ...
   void              CleanupOldLogs()
     {
      ...
      if(find == INVALID_HANDLE)
         return;
      datetime cutoff = TimeCurrent() - (datetime)InpLogKeepDays * 86400;
      do
        {
...
public:
                     CLogger() : m_magic(0), m_dir("GoldTrendEA\\"),
   ...
   void              Notify(const string msg, const bool push = true)
     {
      ...
      if(push)
        {
         if(!SendNotification("[GoldTrendEA] " + msg))
